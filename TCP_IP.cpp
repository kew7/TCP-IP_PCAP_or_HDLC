...
bool CTCP_IP::Soed(BYTE *BufIN,UINT cdata)					// регистрация соединений
{
	BYTE a=0;
	BYTE*pBuf;
	UINT ppp;
...
void CTCP_IP::Zagolovok()  // вставляет заголовок в начало регистрируемого сеанса
{
	...
	for (j = 3;j >= 0;j--)
	{
		zIP = (nData[nCountData].dIP_ot >> (8 * j)) & 0xFF;	// IP-адрес отправителя
		strTemp = strZagFR;
	...
	strTemp.Format("%u", nData[nCountData].dPort_ot);		// TCP-порт отправителя
	...
	strZagFR = " FROM " + strZagFR + " " + strTemp;			// IP-адрес и TCP-порт отправителя в заголовок

	for (j = 3;j >= 0;j--)
	{
		zIP = (nData[nCountData].dIP_po >> (8 * j)) & 0xFF;	// IP-адрес получателя
		strTemp = strZagTO;
	...
	strTemp.Format("%u", nData[nCountData].dPort_po);		// TCP-порт получателя
	switch (strTemp.GetLength())
	{
	case 1: strTemp = "0000" + strTemp; break;
	case 2: strTemp = "000" + strTemp;  break;
	...
	strZagTO = "  TO " + strZagTO + " " + strTemp;			// IP-адрес и TCP-порт получателя в заголовок

	// объединяем части заголовка для получателя и отправителя
...
void CTCP_IP::WriteSeans(UINT& gFile, UINT& gDir, CString& gstrDir,CString strDir, UINT k)
{
	...
	// Расширения регистрирумых файлов - в зависимости от номера использумего порта
